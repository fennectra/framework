...
### `Auth` (app-level, generated by `make:auth`)

Application-level authentication middleware generated by `./forge make:auth`. Uses `JwtService` (resolved from `Container`) instead of manual base64/hash_hmac JWT validation.

**Token validation flow:**
1. Extract Bearer token from the `Authorization` header.
2. Decode and validate the JWT via `JwtService` (from `Container::getInstance()`).
3. Look up the user in the database via `User::findByEmailAndToken($email, $rawToken)`.
4. Check that the user account is active (`is_active`).
5. Store the full `User` object in `$_REQUEST['__auth_user']`.

**Constraint types:**

| Syntax | Effect |
|---|---|
| `[Auth::class, []]` | Authenticated user required (no role/permission check) |
| `[Auth::class, ['role:admin']]` | User must have the `admin` role |
| `[Auth::class, ['permission:users.create']]` | User must have the `users.create` permission |

Multiple constraints can be combined in the array. The permission check follows the chain: `user` -> `user_roles` -> `roles` -> `role_permissions` -> `permissions`.

**Static helper:**

```php
// In any controller, after Auth middleware has run:
...
// Role check
$router->get('/admin', [AdminController::class, 'index'], [
    [Auth::class, ['role:admin']],
]);

// Permission check
$router->post('/users', [UserController::class, 'store'], [
    [Auth::class, ['permission:users.create']],
]);
```
...
| Code | Condition |
|---|---|
| 401 | Missing token or invalid/expired JWT |
| 403 | Account not active, missing role, or missing permission |
...
| `app/Middleware/Auth.php` | App-level JWT auth with role/permission constraints (2026-03-23) |
